...
const buttonReto8 = document.getElementById ("buttonReto8");
...
function ejercicioOcho() {
  const numeros = [30,55, 45, 60, 69, 72, 48, 10];
  let valor = 0;

while (valor < numeros.length) {
  if (numeros[valor] > 50) {
    console.log(`El primer número mayor que 50 es: ${numeros[valor]}`);
    break;
  }
  valor++;
}
...
if (valor === numeros.length) {
  console.log("No se encontró un número mayor que 50.");
}
}
